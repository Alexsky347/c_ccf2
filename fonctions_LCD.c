...
{
    // ajout de  0x80 conform\E9ment \E0 la doc afficheur HD44780 
    LCDPutInst((char)0x80+P_Pos);
}

//P_Ligne 0 ou 1, P_Col 0 \E0 15
...
void lcdClear()
{
     LCDPutInst(LCD_CLEAR); // clear display
}


void lcd_putc(char c) {
   switch (c) {
     case '\f'   : lcdClear(); //efface l'afficheur
                    __delay_ms(5); //Pause de 2ms
                    break;
     case '\n'   : lcdGoToLC(1,0);        break; //saut au d\E9but de la 
                                                 //deuxi\E8me ligne
     case '\b'   : LCDPutInst(0x10);  break;     //backspace.
